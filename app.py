...
def obter_token():
    ...
    if token_response.status_code == 200:
        ...
        if "token" in token_data:
            ...
            st.error(f"Erro ao obter o token: {token_data}")
            return None
    else:
        st.error(f"Erro na requisição do token: {token_response.status_code}")
        return None

# Função para carregar os dados do FeatureLayer
def carregar_dados(token):
    ...
    if response.status_code == 200:
        ...
        st.error("Erro ao carregar os dados do FeatureLayer")
        return pd.DataFrame()
...
def main():
    ...
    if st.button('Carregar Dados'):
        token = obter_token()
        if token:
            dados = carregar_dados(token)
            if not dados.empty:
                # Criar colunas de coordenadas X e Y a partir da geometria
